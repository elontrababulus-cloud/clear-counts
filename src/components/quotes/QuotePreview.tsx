/**
 * QuotePreview
 *
 * A print-ready / PDF-ready layout. Rendered off-screen and captured by
 * html2canvas + jsPDF in QuotePDFExport. Also used inline on the detail page
 * as a read-only preview.
 *
 * Intentionally styled with inline styles where possible so html2canvas
 * faithfully captures the layout without relying on Tailwind CSS variables.
 */

import { format } from 'date-fns';
import type { QuoteDoc, CompanySettings } from '@/types';

// ─── Props ────────────────────────────────────────────────────────────────────

interface QuotePreviewProps {
  quote: QuoteDoc;
  settings?: Partial<CompanySettings> | null;
  /** When true, renders with a visible border and shadow for inline preview */
  inline?: boolean;
}

// ─── Helpers ──────────────────────────────────────────────────────────────────

function fmt(n: number) {
  return n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}

function fmtDate(ts: { seconds: number } | null | undefined) {
  if (!ts) return '—';
  return format(new Date(ts.seconds * 1000), 'MMMM d, yyyy');
}

// ─── Status label ─────────────────────────────────────────────────────────────

const STATUS_LABEL: Record<string, string> = {
  draft: 'DRAFT',
  sent: 'SENT',
  accepted: 'ACCEPTED',
  declined: 'DECLINED',
  deleted: 'VOID',
};

const STATUS_COLOR: Record<string, string> = {
  draft: '#6b7280',
  sent: '#2563eb',
  accepted: '#16a34a',
  declined: '#dc2626',
  deleted: '#9ca3af',
};

// ─── Component ────────────────────────────────────────────────────────────────

export function QuotePreview({ quote, settings, inline }: QuotePreviewProps) {
  const companyName = settings?.name ?? 'ClearCounts';
  const companyEmail = settings?.email ?? '';
  const currency = quote.currency ?? 'USD';

  const subtotal = quote.lineItems.reduce(
    (s, r) => s + r.quantity * r.unitPrice,
    0,
  );
  const taxTotal = quote.taxTotal;
  const discountAmount =
    quote.discountType === 'percent'
      ? subtotal * (quote.discount / 100)
      : (quote.discount ?? 0);

  return (
    <div
      id="quote-preview-root"
      style={{
        fontFamily: 'Inter, system-ui, sans-serif',
        fontSize: '13px',
        color: '#111827',
        background: '#ffffff',
        width: '794px', // A4 width at 96 dpi
        padding: '48px',
        boxSizing: 'border-box',
        ...(inline
          ? { border: '1px solid #e5e7eb', borderRadius: '12px', boxShadow: '0 1px 4px #0001' }
          : {}),
      }}
    >
      {/* ── Header ── */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '36px' }}>
        {/* Branding */}
        <div>
          <div style={{ fontSize: '22px', fontWeight: 700, color: '#0f172a', letterSpacing: '-0.5px' }}>
            {companyName}
          </div>
          {companyEmail && (
            <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>{companyEmail}</div>
          )}
        </div>

        {/* Quote title + status watermark */}
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: '28px', fontWeight: 800, color: '#0f172a', letterSpacing: '-1px' }}>
            QUOTE
          </div>
          <div style={{ fontSize: '13px', color: '#6b7280' }}>{quote.quoteNumber}</div>
          <div
            style={{
              marginTop: '6px',
              display: 'inline-block',
              padding: '2px 10px',
              borderRadius: '99px',
              border: `1.5px solid ${STATUS_COLOR[quote.status] ?? '#6b7280'}`,
              color: STATUS_COLOR[quote.status] ?? '#6b7280',
              fontSize: '10px',
              fontWeight: 700,
              letterSpacing: '0.05em',
            }}
          >
            {STATUS_LABEL[quote.status] ?? quote.status.toUpperCase()}
          </div>
        </div>
      </div>

      {/* ── Meta row ── */}
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: '1fr 1fr',
          gap: '24px',
          marginBottom: '32px',
          padding: '20px 24px',
          background: '#f8fafc',
          borderRadius: '8px',
        }}
      >
        {/* Bill To */}
        <div>
          <div style={{ fontSize: '10px', fontWeight: 600, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>
            Bill To
          </div>
          <div style={{ fontWeight: 600, fontSize: '14px' }}>{quote.clientName}</div>
        </div>

        {/* Dates */}
        <div>
          <MetaRow label="Issue Date" value={fmtDate(quote.createdAt)} />
          <MetaRow label="Valid Until" value={fmtDate(quote.validUntil)} />
          <MetaRow label="Currency" value={currency} />
        </div>
      </div>

      {/* ── Line items table ── */}
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '24px' }}>
        <thead>
          <tr style={{ background: '#0f172a', color: '#ffffff' }}>
            {['Description', 'Qty', 'Unit Price', 'Tax %', 'Amount'].map((h, i) => (
              <th
                key={h}
                style={{
                  padding: '8px 12px',
                  textAlign: i === 0 ? 'left' : 'right',
                  fontSize: '11px',
                  fontWeight: 600,
                  letterSpacing: '0.04em',
                }}
              >
                {h}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {quote.lineItems.map((item, idx) => (
            <tr
              key={item.id}
              style={{ background: idx % 2 === 0 ? '#ffffff' : '#f8fafc' }}
            >
              <td style={{ padding: '9px 12px', borderBottom: '1px solid #e5e7eb' }}>{item.description}</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', borderBottom: '1px solid #e5e7eb' }}>{item.quantity}</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', fontFamily: 'monospace', borderBottom: '1px solid #e5e7eb' }}>{fmt(item.unitPrice)}</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', borderBottom: '1px solid #e5e7eb' }}>{item.taxPercent}%</td>
              <td style={{ padding: '9px 12px', textAlign: 'right', fontFamily: 'monospace', fontWeight: 600, borderBottom: '1px solid #e5e7eb' }}>{fmt(item.total)}</td>
            </tr>
          ))}
        </tbody>
      </table>

      {/* ── Totals ── */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '32px' }}>
        <div style={{ width: '280px' }}>
          <TotalRow label="Subtotal" value={`${currency} ${fmt(subtotal)}`} />
          {discountAmount > 0 && (
            <TotalRow
              label={`Discount${quote.discountType === 'percent' ? ` (${quote.discount}%)` : ''}`}
              value={`− ${currency} ${fmt(discountAmount)}`}
            />
          )}
          <TotalRow label="Tax" value={`${currency} ${fmt(taxTotal)}`} />
          <div
            style={{
              display: 'flex',
              justifyContent: 'space-between',
              padding: '10px 0',
              borderTop: '2px solid #0f172a',
              marginTop: '4px',
              fontWeight: 700,
              fontSize: '15px',
            }}
          >
            <span>Total</span>
            <span style={{ fontFamily: 'monospace' }}>{currency} {fmt(quote.total)}</span>
          </div>
        </div>
      </div>

      {/* ── Footer: notes + terms ── */}
      {(quote.notes || quote.terms) && (
        <div
          style={{
            borderTop: '1px solid #e5e7eb',
            paddingTop: '20px',
            display: 'grid',
            gridTemplateColumns: quote.notes && quote.terms ? '1fr 1fr' : '1fr',
            gap: '24px',
          }}
        >
          {quote.notes && (
            <div>
              <div style={{ fontSize: '10px', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>
                Notes
              </div>
              <div style={{ fontSize: '12px', color: '#374151', lineHeight: 1.6 }}>{quote.notes}</div>
            </div>
          )}
          {quote.terms && (
            <div>
              <div style={{ fontSize: '10px', fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '6px' }}>
                Payment Terms
              </div>
              <div style={{ fontSize: '12px', color: '#374151', lineHeight: 1.6 }}>{quote.terms}</div>
            </div>
          )}
        </div>
      )}

      {/* ── Generated by footer ── */}
      <div style={{ marginTop: '32px', textAlign: 'center', fontSize: '10px', color: '#9ca3af' }}>
        Generated by ClearCounts CRM
      </div>
    </div>
  );
}

// ─── Sub-components ───────────────────────────────────────────────────────────

function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', gap: '8px', marginBottom: '4px', fontSize: '12px' }}>
      <span style={{ color: '#6b7280', minWidth: '80px' }}>{label}:</span>
      <span style={{ fontWeight: 500 }}>{value}</span>
    </div>
  );
}

function TotalRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid #f1f5f9', fontSize: '13px' }}>
      <span style={{ color: '#6b7280' }}>{label}</span>
      <span style={{ fontFamily: 'monospace' }}>{value}</span>
    </div>
  );
}
